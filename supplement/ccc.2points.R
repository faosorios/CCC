## Id: ccc.2points.R, last updated 2019/01/08
## Author: originally coded by Carla Leal, with contributions of Felipe Osorio

simul2points.ccc <- function(Nsize = 500, nobs = 100, mean, Sigma, delta = 3)
{ ## function to perform the additional simulation experiment (Supplementary Material)
...
summary2.ccc <- function(Nsize = 500, nobs = 100, mean, Sigma) {
  out <- matrix(0, nrow = 5, ncol = 7)

  set.seed(5)
  out[,1] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 0.5)$percentage
  set.seed(23)
  out[,2] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 1.0)$percentage
  set.seed(53)
  out[,3] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 1.5)$percentage
  set.seed(167)
  out[,4] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 2.0)$percentage
  set.seed(239)
  out[,5] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 2.5)$percentage
  set.seed(347)
  out[,6] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 3.0)$percentage
  set.seed(433)
  out[,7] <- simul2points.ccc(Nsize, nobs, mean, Sigma, delta = 3.0)$percentage
