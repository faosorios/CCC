...
{ ## function to perform the simulation experiment (Section 4 of the manuscript)
  ...
  for (i in 1:Nsize) {
    ...
    # largest slope
    z <- influence.ccc(fm, method = "CF")
    hmax <- abs(z$hmax)
    cutoff[3] <- mean(hmax) + 2 * sd(hmax)
    ok[i,3] <- hmax[1] > cutoff[3]
    # FI measure
    z <- influence.ccc(fm, method = "FI")
    hmax1 <- abs(z$hmax)
    cutoff[4] <- mean(hmax1) + 2 * sd(hmax1)
    ok[i,4] <- hmax1[1] > cutoff[4]
    # SI measure
    z <- influence.ccc(fm, method = "conformal")
    hmax2 <- abs(z$hmax)
    cutoff[5] <- mean(hmax2) + 2 * sd(hmax2)
    ok[i,5] <- hmax2[1] > cutoff[5]
    # FI & SI measures
    ok[i,6] <- (hmax1[1] > cutoff[4]) && (hmax2[1] > cutoff[5])
  }

  mnames <- c("C","B","CF","FI","SI","both")
  percentage <- apply(ok, 2, sum) / Nsize
  names(percentage) <- mnames
  names(cutoff) <- mnames[1:5]
  speed <- proc.time() - now
